(function( $ ){
	var methods = {
		init: function(options){
			...
			return this.each(function () {
				...
				if(!$(this).hasClass('multinput-init')){
	        		...
	        		//var input = $('<textarea style="display:inline-block;border:none;resize: none;"></textarea>').insertAfter(this);
	        		if(!$(this).prop('disabled')){
		        		var input = $('<textarea class="ui-widget" style="display:inline;border:none;resize:none;overflow:auto"></textarea>').appendTo(li);
						...
						input.on("paste", function(e) {
							var to = window.setTimeout(function(){
								...
								$(thisObj).trigger({type:"paste", pastedData:data});
								input.val('');
							}, 0);
						});
						...
						//var input = $('<textarea style="display:inline;border:none;resize:none;overflow:auto"></textarea>').appendTo(li);
						var input = $('<div style="clear:both;display:block;border:none;resize:none;overflow:auto"></div>').appendTo(li);
					}
		...
		addListItem: function(opt){
			...
			var self = this;
			var li = $('<li class="multinput-li"></li>').prependTo(list);
			li.data('option', opt);
			
			if(!$(this).prop('disabled')){
				var cancel = $('<span class="remove">x</span>').appendTo(li)
				...
				var cancel = $('<span>-</span>').appendTo(li)
			}
